    ...
    def set_reference(self, path):
        ...
        if img1 is not None:
            ...
            if "MONA LISA" in path.upper():
                da_vinci_path = (
                    path.replace("MONA LISA.png", "DAVICI.png")
                    .replace("mona_lisa", "Davici")
                )
            ...
            # If Davici, add Monalisa sequence
                ...
                mona_lisa_path = (
                    path.replace("DAVICI.png", "MONA LISA.png")
                    .replace("Davici", "mona_lisa")
                )
            ...
            if self.ref_loaded:
                ...
                if x1 > x0 and y1 > y0:
                    ...
                    if crop_pts is not None:
                        ...
                        if self.seq_index == 0:
                            # Stage 0: Camera -> First Target
                            progress = min(1.0, elapsed / self.duration)

                            img_B, pts_B = self.sequence_data[0]
                            morphed_B = self.warp_ref_to_detected(
                                crop, crop_pts, img_B, pts_B
                            )
                            morphed_A = crop.astype(np.float32)
                            blended = cv2.addWeighted(
                                morphed_A, 1.0 - progress,
                                morphed_B, progress,
                                0.0
                            )
                        else:
                            # Stage 1: Target 1 -> Target 2
                            progress = min(1.0, elapsed / self.duration)
                            img_A, pts_A = self.sequence_data[0]
                            img_B, pts_B = self.sequence_data[1]
                            morphed_A = self.warp_ref_to_detected(
                                crop, crop_pts, img_A, pts_A
                            )
                            morphed_B = self.warp_ref_to_detected(
                                crop, crop_pts, img_B, pts_B
                            )
                        ...
                        target_frames = 9
                        expected_captures = min(
                            target_frames,
                            int(global_progress * target_frames) + 1
                        )

                        while (
                            len(self.captured_frames) < expected_captures
                            and len(self.captured_frames) < target_frames
                        ):
                            snap = blended.copy()
                            cv2.putText(
                                snap,
                                f"{int(global_progress * 100)}%",
                                (10, 30),
                        ...
                        morphed_back = cv2.resize(
                            blended,
                            (base_crop.shape[1], base_crop.shape[0])
                        )

                        # Create an edge-blending mask for lower opacity along edges
                        ...
                        blended_bg = (
                            morphed_f * mask + original_bg * (1.0 - mask)
                        ).astype(np.uint8)

                        # Blend the morphed face into the original frame
